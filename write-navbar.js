...
const oldEnd = content.lastIndexOf('</header>');
        ...
        {mobileOpen && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 1100 }}>
            ...
            <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 'min(88vw,360px)', background: 'linear-gradient(160deg,#080f1c 0%,#0a1929 40%,#0d1f35 100%)', borderLeft: '1px solid rgba(14,165,233,.18)', boxShadow: '-32px 0 80px rgba(0,0,0,.6)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
              <div style={{ padding: '18px 18px 14px', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '34px', height: '34px', borderRadius: '10px', background: 'linear-gradient(135deg,#0ea5e9,#38bdf8)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 14px rgba(14,165,233,.4)' }}>
              ...
              <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <p style={{ fontSize: '9px', fontWeight: 800, color: '#0ea5e9', letterSpacing: '.22em', textTransform: 'uppercase', marginBottom: '8px', paddingLeft: '2px' }}>Certification Paths</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    {courseItems.map((course) => (
                      <Link key={course.exam} href={course.href} onClick={() => setMobileOpen(false)}
                        style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 13px', borderRadius: '11px', background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.05)', textDecoration: 'none', transition: 'all .18s' }}
                        ...
                        <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: course.color, boxShadow: '0 0 8px ' + course.color + '70', flexShrink: 0 }} />
                ...
                {!user && (
                <div>
                  <p style={{ fontSize: '9px', fontWeight: 800, color: '#475569', letterSpacing: '.22em', textTransform: 'uppercase', marginBottom: '8px', paddingLeft: '2px' }}>Explore</p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px' }}>
                    {[
                      ...
                      <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)}
                        style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.05)', textDecoration: 'none', transition: 'all .18s' }}
                ...
                {user && (
                <div>
                  <p style={{ fontSize: '9px', fontWeight: 800, color: '#0ea5e9', letterSpacing: '.22em', textTransform: 'uppercase', marginBottom: '8px', paddingLeft: '2px' }}>My Study Portal</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    {[
                      ...
                      <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)}
                        style={{ display: 'flex', alignItems: 'center', gap: '13px', padding: '11px 13px', borderRadius: '11px', background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.05)', textDecoration: 'none', transition: 'all .18s' }}
                        onMouseEnter={e => { e.currentTarget.style.background = item.color + '10'; e.currentTarget.style.borderColor = item.color + '25'; }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,.03)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,.05)'; }}>
              ...
              <div style={{ padding: '14px 16px', borderTop: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
                {user ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <Link href='/auth/login' onClick={() => setMobileOpen(false)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.09)', fontSize: '13px', fontWeight: 600, color: '#94a3b8', textDecoration: 'none' }}>
                      Sign in
                    </Link>
