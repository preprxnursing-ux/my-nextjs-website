...
const fontStyle = `
  ...
  *, body { font-family: 'Plus Jakarta Sans', sans-serif; }
  ...
  input:-webkit-autofill {
    -webkit-box-shadow: 0 0 0 30px #0a1929 inset !important;
              ...
              <div className="fade-up" style={{ position: "relative", animationDelay: ".2s" }}>
                <div style={{ borderRadius: "24px", overflow: "hidden", height: "420px", position: "relative" }}>
                  ...
                  <div style={{ position: "absolute", bottom: "24px", right: "24px", background: "rgba(6,15,30,.92)", backdropFilter: "blur(12px)", border: "1px solid rgba(139,92,246,.2)", borderRadius: "16px", padding: "16px 20px" }}>
                    <p style={{ fontSize: "11px", color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".1em", margin: "0 0 6px" }}>Team Size</p>
            ...
            {submitted ? (
              <div style={{ textAlign: "center", padding: "60px 0" }}>
                ...
                <p style={{ color: "#64748b", lineHeight: 1.8, maxWidth: "400px", margin: "0 auto 28px" }}>
                  Thank you for reaching out. {selectedContact.name} will get back to you {selectedContact.responseTime.toLowerCase()}.
                </p>
                <button onClick={() => { setSubmitted(false); setFormData({ name: "", email: "", subject: "", message: "", to: "melissa" }); }}
                  style={{ padding: "12px 28px", borderRadius: "12px", background: "rgba(14,165,233,.15)", border: "1px solid rgba(14,165,233,.3)", color: "#38bdf8", fontSize: "14px", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all .2s" }}
                  onMouseEnter={e => { e.currentTarget.style.background = "rgba(14,165,233,.25)"; }}
                  onMouseLeave={e => { e.currentTarget.style.background = "rgba(14,165,233,.15)"; }}>
              ...
              <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: "24px", padding: "40px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    {contacts.map(c => (
                      <button key={c.name} onClick={() => setFormData(prev => ({ ...prev, to: c.name.toLowerCase() }))}
                        ...
                        <div style={{ width: "44px", height: "44px", borderRadius: "50%", overflow: "hidden", border: `2px solid ${formData.to === c.name.toLowerCase() ? c.color + "60" : c.color + "25"}`, flexShrink: 0, transition: "border-color .25s" }}>
                          <img src={c.avatar} alt={c.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                        </div>
                        <div style={{ textAlign: "left", flex: 1 }}>
                          <p style={{ fontSize: "14px", fontWeight: 700, color: formData.to === c.name.toLowerCase() ? "#f8fafc" : "#94a3b8", margin: "0 0 3px", transition: "color .25s" }}>{c.name}</p>
                          <p style={{ fontSize: "11px", color: formData.to === c.name.toLowerCase() ? c.color : "#334155", margin: 0, fontWeight: 600, transition: "color .25s" }}>{c.role}</p>
                          <p style={{ fontSize: "10px", color: "#334155", margin: "2px 0 0", fontWeight: 400 }}>{c.responseTime}</p>
                        </div>
                        {formData.to === c.name.toLowerCase() && (
                ...
                <div style={{ marginBottom: "28px" }}>
                  ...
                  <p style={{ fontSize: "11px", color: "#334155", marginTop: "6px" }}>
                    This will be sent to {selectedContact.name} at {selectedContact.email}
                  </p>
                </div>
        ...
        <section style={{ background: "linear-gradient(180deg,#0c1e35 0%,#0a1929 100%)", padding: "80px 20px" }}>
          <div style={{ maxWidth: "760px", margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: "48px" }}>
              ...
              <p style={{ fontSize: "15px", color: "#64748b", marginTop: "10px" }}>You might find your answer here first.</p>
            </div>
              ...
              {faqs.map((faq, i) => (
                <div key={i} className="faq-item"
                  ...
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                    ...
                    <p style={{ fontSize: "15px", fontWeight: 700, color: openFaq === i ? "#38bdf8" : "#e2e8f0", margin: 0, transition: "color .2s", lineHeight: 1.4 }}>{faq.q}</p>
                  ...
                  {openFaq === i && (
                    <div style={{ padding: "0 24px 20px" }}>
                      <p style={{ fontSize: "14px", color: "#94a3b8", lineHeight: 1.8, margin: 0, fontWeight: 400 }}>{faq.a}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          <div style={{ position: "relative", zIndex: 1, maxWidth: "600px", margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
              <Link href="/auth/signup" style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "linear-gradient(135deg,#0ea5e9,#38bdf8)", color: "#fff", padding: "15px 36px", borderRadius: "12px", fontSize: "15px", fontWeight: 700, textDecoration: "none", boxShadow: "0 8px 28px rgba(14,165,233,.35)", transition: "all .3s cubic-bezier(.34,1.56,.64,1)" }}
                ...
                onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0) scale(1)"; e.currentTarget.style.boxShadow = "0 8px 28px rgba(14,165,233,.35)"; }}>
                Start free today →
              </Link>
  ...
  return (
    <div ref={ref} className="contact-card"
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "20px" }}>
        {c.topics.map(t => (
          <span key={t} style={{ fontSize: "11px", fontWeight: 600, background: `${c.color}12`, color: c.color, border: `1px solid ${c.color}25`, padding: "5px 12px", borderRadius: "100px", transition: "all .3s", ...(hovered ? { background: `${c.color}20`, borderColor: `${c.color}45` } : {}) }}>
            {t}
          </span>
        ))}
      </div>
      ...
      <a href={`mailto:${c.email}`}
        style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", width: "100%", padding: "14px", borderRadius: "14px", background: hovered ? c.color : `${c.color}15`, color: hovered ? "#fff" : c.color, border: `1px solid ${c.color}${hovered ? "ff" : "40"}`, textDecoration: "none", fontSize: "13px", fontWeight: 700, transition: "all .35s cubic-bezier(.34,1.56,.64,1)", boxShadow: hovered ? `0 8px 28px ${c.color}40` : "none", transform: hovered ? "scale(1.02)" : "scale(1)" }}>
