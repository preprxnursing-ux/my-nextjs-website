          ...
          <div className="w-full overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl">
            <div className="border-b border-slate-200 bg-slate-50 p-8 md:p-10">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600 md:text-base">
                Review your results, inspect rationales, or restart with a fresh
                attempt.
              </p>
            </div>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="mt-8 grid gap-3 md:grid-cols-3">
                <Link
                  href="/results"
                  className="rounded-2xl bg-slate-900 px-5 py-3 text-center font-semibold text-white transition hover:opacity-90"
                >
                  View Results
                </Link>
                ...
                <Link
                  href="/review"
                  className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-center font-semibold text-slate-900 transition hover:bg-slate-50"
                >
                  Review Answers
                </Link>
                ...
                <button
                  ...
                  className="rounded-2xl border border-slate-300 bg-slate-50 px-5 py-3 font-semibold text-slate-900 transition hover:bg-slate-100"
                >
                  Restart Exam
                </button>
              </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="mb-3 flex flex-wrap gap-2">
                    ...
                    <ModeBadge>
                      {mode.tutorMode ? "Guided feedback" : "Exam simulation"}
                    </ModeBadge>
                  </div>

                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                    NCLEX Practice Workspace
                  </p>
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    {mode.tutorMode
                      ? "Check answers, read rationale, then move with confidence."
                      : "Stay focused, answer decisively, and submit when ready."}
                  </p>
                </div>
            ...
            <div className="p-6 md:p-8">
              {mode.tutorMode && (
                <div className="mb-6 rounded-3xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
                  Tutor Mode is active. You can check each answer, see feedback,
                  and learn question by question.
                </div>
              )}

              {!mode.tutorMode && (
                <div className="mb-6 rounded-3xl border border-sky-200 bg-sky-50 p-4 text-sky-800">
                  Timed exam mode is active. You can select answers, navigate the
                  session, and submit when ready.
                </div>
              )}
              ...
              <div className="grid gap-4 md:grid-cols-4">
                ...
                />
                <TopMetric
                  label={mode.tutorMode ? "Checked" : "Answered"}
                  value={String(mode.tutorMode ? checkedCount : selectedCount)}
                />
              ...
              <div className="mt-6 space-y-4">
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between text-sm text-slate-600">
                    <span>{mode.tutorMode ? "Learning completion" : "Answer completion"}</span>
                    <span>{completionPercent}%</span>
                  </div>
                  <div className="h-3 w-full overflow-hidden rounded-full bg-slate-200">
                    <div
                      className={`h-full rounded-full transition-all ${completionBarClass}`}
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                ...
                <Badge className={performanceBadgeClass}>
                  {accuracyPercent >= 70 ? "On track" : "Keep improving"}
                </Badge>
              ...
              <div className="mt-8 rounded-3xl border border-slate-200 bg-slate-50 p-5 md:p-6">
                ...
                <h2 className="mt-3 text-xl font-semibold leading-8 text-slate-900 md:text-2xl">
              ...
              <div className="mt-6 space-y-3">
                {currentQuestion.options.map((option, index) => {
                  ...
                  if (mode.tutorMode && showFeedback) {
                    if (index === resolvedCorrectAnswer) {
                      optionClass +=
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-left text-sm font-medium leading-7 md:text-base">
                            {option}
                          </p>
                        </div>
                      </div>
                ...
                >
                  <p
                    className={`text-sm font-semibold uppercase tracking-[0.18em] ${
                      currentIsCorrect ? "text-emerald-700" : "text-rose-700"
                    }`}
                  >
                  ...
                  <p className="mt-3 text-sm leading-7 text-slate-700">
                    ...
                    {currentQuestion.options[getCorrectAnswer(currentQuestion)]}
                  </p>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <button
                  ...
                  className="w-full rounded-2xl border border-slate-300 bg-white px-5 py-3 font-medium text-slate-800 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:min-w-[140px]"
                >
                  Previous
                </button>
                ...
                <button
                  ...
                  className={`w-full rounded-2xl px-5 py-3 font-medium transition sm:w-auto sm:min-w-[160px] ${
                    flaggedQuestions.includes(currentIndex)
                ...
                {mode.tutorMode && !showFeedback ? (
                  <button
                    ...
                    className="w-full rounded-2xl bg-slate-900 px-5 py-3 font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 sm:flex-1"
                  >
                    Check Answer
                  </button>
                  ...
                  <button
                    ...
                    className="w-full rounded-2xl bg-slate-900 px-5 py-3 font-medium text-white transition hover:opacity-90 sm:flex-1"
                  >
                    Submit Exam
                  </button>
                ) : (
                ...
                <button
                  onClick={() => finishExam(false)}
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-5 py-3 font-medium text-slate-800 transition hover:bg-slate-100 sm:w-auto sm:min-w-[140px]"
                >
                  End Exam
                </button>
              </div>
          ...
          <aside className="h-fit rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl xl:sticky xl:top-24">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600">
              Jump to any question, monitor progress, flag items for later, and
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-5 gap-2">
                {activeQuestions.map((_, index) => {
                  ...
                  const selectedButUnchecked =
                    selectedMap[index] !== undefined && !checkedMap[index];
            ...
            <div className="mt-6 space-y-3">
              <button
                onClick={handleRestart}
                className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 font-medium text-slate-800 transition hover:bg-slate-50"
              >
                Restart Exam
              </button>
              ...
              <Link
                href="/review"
                className="block w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-center font-medium text-slate-800 transition hover:bg-slate-100"
              >
                Open Review Page
              </Link>
            </div>
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700 ${className}`}
    >
      {children}
    </span>
  );
...
function LegendItem({
  ...
  return (
    <div className="flex items-center gap-3">
      <span
        className={`inline-flex h-4 w-4 rounded-full ${tone} ${
          dark ? "border border-slate-900" : "border border-slate-200"
        }`}
      />
      <span>{label}</span>
    </div>
